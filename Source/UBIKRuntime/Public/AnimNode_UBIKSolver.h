...
private:
	...
	void SolveArms();
	void SetElbowBasePosition(const FVector& UpperArm, const FVector& Hand, bool bIsLeftArm, FTransform& UpperArmTransform, FTransform& LowerArmTransform);
	...
	void RotateElbow(float Angle, const FTransform& UpperArm, const FTransform& LowerArm, const FVector& HandLoc, bool bIsLeftArm, FTransform& NewUpperArm, FTransform& NewLowerArm);

	FTransform GetBaseCharTransform();

	void DrawDebug(FAnimInstanceProxy* AnimInstanceProxy);
	void DebugDrawAxes(FAnimInstanceProxy* AnimInstanceProxy, const FTransform& Transform, bool DrawAxis = true, FColor SphereColor = FColor::Silver);
	
	/** The given Transform will only apply Translation if explicitly defined. */
	void SetBoneTransform(TArray<FBoneTransform>& OutBoneTransforms, const FBoneReference& BoneToModify, const FTransform& InTransform, FComponentSpacePoseContext& Output, const FBoneContainer& BoneContainer, bool bApplyRotation, bool bApplyTranslation = false);
